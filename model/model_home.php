    ...
    function getReportNarrative()
  	{
  		...
  		$this->db = Db::getInstance();
  		//$db->setAttribute(PDO::ATTR_EMULATE_PREPARES, FALSE);
          $sQuery   = "select report_narrative from kb_CancerVariant_Curation.CVC_viewer_admin where gene = '".$gene."' and variant = '".$variant."'  and cancer = '".$cancer."'  order by date_admin desc limit 1";
  	    //$stmt     = $this->db->prepare($sQuery);
  		//echo $sQuery;
  	//	$stmt->bindParam(':cancer', $cancer);
         // $stmt->bindParam(':gene', $gene);
         // $stmt->bindParam(':variant', $variant);
  		...
  		if ($rowcount ==1) {
  			...
  			$stmt = $this->db->prepare("INSERT INTO CVC_viewer_admin (cancer, gene,variant,report_narrative,date_admin,ver_name) VALUES (:cancer, :gene,:mutation,:report_narrative,:date_admin,:ver_name)");
              $stmt->bindParam(':cancer', $cancer);
              $stmt->bindParam(':gene', $gene);
              $stmt->bindParam(':mutation', $mutation);
              $stmt->bindParam(':ver_name', $ver_name);
             // $stmt->bindParam(':uid', $uid);
              $stmt->bindParam(':date_admin', $date_admin);
              $stmt->bindParam(':report_narrative', $report_narrative);
              $mutation   = $_POST["variant"];
              $cancer     = $_POST["cancer"];
              $gene       = $_POST["gene"];
              $ver_name   = "original";
              //$uid        = $_POST["uid"];
              //$narrative  = $_POST["narrative"];
  			$narrative=$this->getReportNarrative_origin();
  			if($report_narrative!="1"){
  				echo $report_narrative;
      ...
      function getReportNarrative_origin()
      {
          $result = "";
          $this->db = Db::getInstance();
          $sQuery   = "select report_narrative from kb_CancerVariant_Curation.CVC_viewer where gene = :gene and variant = :variant and cancer = :cancer";
  	    $stmt     = $this->db->prepare($sQuery);
  		$stmt->bindParam(':cancer', $cancer);
          $stmt->bindParam(':gene', $gene);
          $stmt->bindParam(':variant', $variant);
      ...
      ////////////////
      function modifyReportNarrative()
      {
          $status   = $_POST["status"];
          $this->db = Db::getInstance();
          if ($status == 1) {
              $sql       = "UPDATE CVC_viewer_admin SET  report_narrative= :report_narrative,date_admin = :date_admin WHERE  ver_name = :ver_name and cancer= :cancer and gene = :gene and varaint = :mutation";
              $statement = $pdo->prepare($sql);
              $stmt->bindValue(':gene', $gene);
              $stmt->bindValue(':mutation', $mutation);
              $stmt->bindValue(':cancer', $cancer);
              $statement->bindValue(':report_narrative', $report_narrative);
              $statement->bindValue(':ver_name', $ver_name);
              $statement->bindValue(':date_admin', $date_admin);
              ...
              $ver_name   = $_POST["ver_name"];
              $uid        = $_POST["uid"];
              $narrative  = $_POST["report_narrative"];
              $date_admin = date('Y-m-d H:i:s');
              try {
                  $stmt->execute();
              }
