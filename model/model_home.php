...
class Tumor_Model extends model_base
{
    ...
    */
    ...
    public function getNarrative_origin()
    {
        ...
        $this->db = Db::getInstance();
        if ($report == 1) {
            $sQuery = "select report_style from CVC_viewer_copy where gene = :gene and variant = :variant and cancer = :cancer";
                //  echo  file_put_contents("origin.txt", $sQuery, true);
        } else {
            $sQuery = "select narrative from CVC_viewer_copy where gene = :gene and variant = :variant and cancer = :cancer";
          //  echo  file_put_contents("origin.txt", $sQuery, true);
