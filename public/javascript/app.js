...
/* 02/09/18
 * Gets genes list that correspond to tissue
 *
 * adds genes to dropdown list
 * @var the celllist is split because after the space is the value of the oncotree
 * Using the gene value than the aleration value  an be chosen
 **/
 ...
 * @var result  for parse errors
 * @var flagArray splits the data in alteration with the notification number for if it has report style or pre-narrative
 * uses different cases to determine the symbols needed
 */
function notifyNarrativeTable(flagMutation) {
  var result = "notifyNarrativeTable_parse error";
 ...
 * @var constructHtml
 * @var mutations_w  calls on notifyNarrativeTable function for each mutation(alteration)
 */
function constructHtml(groupObj) {
  ...
  $.each(groupObj, function(group, mutations) {
    ...
    for (var i = 0; i < mutations.length; i++) {
      ...
      if (mutations_w == "notifyNarrativeTable_parse error") {
        continue;
      }
...
/*
 *11/29/17
 * Adds mutation(alteratons) list  from database into select drop down box
 *
 * @function puts mutations(alterations) into select drop down box
 * @var celllineList we get all the alterations in the list
 * @var ddl is value in the drop down boxes
 * var groupselectHtml calls on getGroups function using the mutation list and regex
 */
 ...
 * @var tissue list list of tumors seperated by ("\n"))
 * @var ddl gets tumor list into dropdown box
 */
function addList() {
  $.ajax({
    type: 'POST',
 ...
 * @var comment value of comment inside textarea box
 */
 ...
 * @var commentObj
 * @var cols colors for viewing comments
 * @var uidindex starts at 0 and will be used in loop for colors of different users
 * @var objarray set to data1
 */
 ...
 *
 * @function is used in getAjaxMessage to view a comment
 */
 ...
 * IN EDIT MODE and ADMIN:
 *
 * @function Generates html content from database used in adminModify
 * @var mtext
 * @var html try commenting out
 */
...
// try commenting out  not sure if its used
 ...
 * @var newUrl
 * @var n
 * @var table loads narrative table using the narrativelist
 */
 ...
 * @var objcount changes color depnding on whats selected
 */
 ...
 * @var rowCount
 * @var colCount
 * @var aver version number
 * @var buttonHtml html clickable button with version name in text
 */
 ...
 * @var dt new date variable
 * @var time time with version than exact date down to seconds
 */
function openDialog() {
  var dt = new Date();
 ...
 * @var mutationFlagArray splits mutation from flag
 * @var url uses api to cancer variant curation
 */
